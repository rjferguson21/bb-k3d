...
- `k3d.serverHost` value to set `kubeAPI.host` and an extra TLS SAN, so kubectl can reach the API server when the docker host is remote

## [0.3.4]

### Changed

- Updated Flux helm chart dependency: 2.18.3 → 2.19.0
...
- Updated Flux controller versions to match Big Bang:
  - helm-controller: v1.5.4 → v1.6.3
  - kustomize-controller: v1.8.4 → v1.9.4
  - notification-controller: v1.8.4 → v1.9.3
  - source-controller: v1.8.3 → v1.9.4

## [0.3.2]

### Added

- Set `--requeue-dependency=10s` on the helm-controller (via `flux.helmController.container.additionalArgs`) to speed up dependent HelmRelease reconciliation
